    ...
    <div className="container max-w-7xl mx-auto py-8 px-4 animate-in fade-in duration-500">
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold mb-2">Analytics de Assertividade</h1>
            <p className="text-muted-foreground">Análise detalhada da performance dos bots de trading</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="bg-card border border-border rounded-lg px-4 py-2 flex items-center gap-2">
              ...
              <span className="text-sm">Todos os bots</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Assertividade Média</p>
            ...
            <div className={cn(
              ...
              <span>{Math.abs(monthlyChange).toFixed(1)}% em relação ao mês anterior</span>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Melhor Assertividade</p>
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Menor Assertividade</p>
            ...
            <div className="text-xs text-muted-foreground">
              <span>Melhoria necessária em {bots.filter(bot => bot.accuracy === lowestAssertiveness).length} bots</span>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            ...
            <div className="text-xs text-muted-foreground">
              <span>Média de {Math.round(totalOperations / bots.length)} por bot</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold">Histórico de Assertividade</h2>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold">Assertividade por Estratégia</h2>
            <div className="text-xs text-muted-foreground">
              Baseado em dados de {bots.length} bots ativos
            </div>
          </div>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value, name, props) => {
                    if (name === 'value') {
                      return [`${value}%`, 'Assertividade'];
                    }
                    return [value, name];
                  }}
                  labelFormatter={(value) => `Estratégia: ${value}`}
                />
                <Legend formatter={(value) => value === 'value' ? 'Assertividade (%)' : value} />
                <Bar 
                  dataKey="value" 
                  name="Assertividade" 
                  radius={[5, 5, 0, 0]}
            ...
            {topPerformers.map((bot, index) => (
              <div key={index} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{bot.strategy} • {bot.operations} operações</p>
                </div>
                <div className={cn(
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl p-6 border border-primary/20">
          ...
          <h3 className="text-lg font-semibold mb-2">Tendências Observadas</h3>
          <p className="text-sm text-muted-foreground mb-2">A assertividade média dos bots vem crescendo nos últimos 6 meses, com aumento de 20% no período.</p>
          <p className="text-xs">Estratégias de Moving Average apresentaram os melhores resultados.</p>
        </div>
        
        <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl p-6 border border-primary/20">
          <Target size={24} className="text-primary mb-4" />
          <h3 className="text-lg font-semibold mb-2">Oportunidades de Melhoria</h3>
          <p className="text-sm text-muted-foreground mb-2">Bots com estratégia Scalping apresentam potencial para otimização, com assertividade atual de 55%.</p>
          <p className="text-xs">Recomendação: ajustar parâmetros de entrada e saída.</p>
        </div>
        
        <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl p-6 border border-primary/20">
          <Award size={24} className="text-primary mb-4" />
          <h3 className="text-lg font-semibold mb-2">Próximos Passos</h3>
          <p className="text-sm text-muted-foreground mb-2">Implementar análise de correlação entre assertividade e rentabilidade para determinar eficiência real.</p>
          <p className="text-xs">Desenvolver sistema de alerta para quedas na assertividade.</p>
        </div>
      </div>
    </div>
  );
